...
--
package body STM32.DS3231 is

   procedure Read_All (This : DS3231_Device; Val : out All_Registers) is
      use HAL.I2C;
   ...
   function To_Integer (BCD : HAL.UInt8) return Integer is
      N : constant Integer :=
        Integer (BCD and 16#F#) + (Integer (Shift_Right (BCD, 4)) * 10);
   begin
      return N;
   end To_Integer;

   function To_BCD (N : Integer) return HAL.UInt8 is
     (Shift_Left (UInt8 (N / 10), 4) or UInt8 (N mod 10));

   overriding procedure Set
     (This : in out DS3231_Device; Time : RTC_Time; Date : RTC_Date)
   is
      use HAL.I2C;
   ...
   overriding procedure Get
     (This : in out DS3231_Device; Time : out RTC_Time; Date : out RTC_Date)
   is
      Val : All_Registers;
      N   : Integer;
   begin
   ...
   overriding function Get_Time (This : DS3231_Device) return RTC_Time is
      Val  : All_Registers;
      Time : RTC_Time;
   begin
      Read_All (This, Val);
   ...
   overriding function Get_Date (This : DS3231_Device) return RTC_Date is
      Val  : All_Registers;
      Date : RTC_Date;
   begin
      Read_All (This, Val);
