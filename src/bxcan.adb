...
procedure bxcan is
   ...
   Period              : constant Time_Span  := Milliseconds (222);
...
begin
   ...
   declare
      ...
      Filter2 : constant CAN_Filter_Config :=
        (Bank => 2, FIFO => 0, Mode => List, ID_Type => STM32.CAN.Standard,
         Id1        => 16#405#, Id2 => 16#000#, Mask1 => 0, Mask2 => 0,
         Use_32_Bit => True);
   begin
   ...
   -- Top Row
   Draw_Info (Point => (X => 9, Y => 122), Text => "TBD");
   Draw_Info (Point => (X => 128, Y => 122), Text => "TBD");
   Draw_Info (Point => (X => 248, Y => 122), Text => "TBD");
   Draw_Info (Point => (X => 367, Y => 122), Text => "TBD");
   ...
   loop
      ...
      GUI.Images.Draw_Image
        (X0            => 230, Y0 => 50, Image => GUI.Images.Steering_Wheel,
